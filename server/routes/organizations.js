...
/**
 * POST /api/organizations/:id/job-seekers/ai-search/stream
 * SSE streaming variant of org-scoped AI search.
 * Emits stage / criteria / results / error / done events.
 */
router.post('/:id/job-seekers/ai-search/stream', authenticateToken, requireRole(['SuperAdmin', 'Admin', 'AdminEvent']), async (req, res) => {
    ...
    try {
        ...
        if (!isValidObjectId(id)) {
            send('error', { error: 'Invalid organization ID' });
            return res.end();
        }
        if (req.user.role !== 'SuperAdmin' && req.orgId?.toString() !== id) {
        ...
        if (error.code === 'SENSITIVE_QUERY') {
            ...
            logger.error('Error in ai-search stream:', error);
